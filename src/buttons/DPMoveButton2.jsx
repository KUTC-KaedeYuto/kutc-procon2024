...
export default function DPMoveButton2({ problem, controller }) {
  ...
  const find = (x, y) => {
    ...
    const cost = new Array(controller.board.cells.length).fill(0).map((_n, i) => 
      new Array(controller.board.cells[i].length).fill(Infinity)
    );
    cost[y][x] = 0;
    while (queue.length > 0) {
      const now = queue.shift();
      
      const nx = now.x, ny = now.y, now_cost = cost[ny][nx];
      try{
        if(ny < y || (ny === y && nx < x)) continue;
        const value = controller.board.cells[ny][nx];
        
        if(value === undefined) throw new Error();
        if(value === num) {
          return [nx, ny];
        };
        const pos = [[nx - 1, ny], [nx + 1, ny], [nx, ny - 1], [nx, ny + 1]];
        for(const p of pos){
          try{
            if(cost[p[1]][p[0]] > now_cost + 1){
              queue.push({
                x: p[0],
                y: p[1]
              });
              cost[p[1]][p[0]] = now_cost + 1;
            } 
          }catch (e){
